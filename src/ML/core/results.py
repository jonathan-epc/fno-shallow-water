...
TRAINED_MODELS_INFO = {
    ...
    "ddn": {
        "trial_number": 194,
        "study_name": "study29ddb",  # Accommodate legacy checkpoint naming convention
        "source_geom": "n",
        "inputs": ["H0", "Q0", "n", "nut", "B"],
        "outputs": ["H", "U", "V"],
    },
    "idn": {
        "trial_number": 94,
        "study_name": "study33ids",  # Accommodate legacy checkpoint naming convention
        "source_geom": "n",
        "inputs": ["U", "V"],
        "outputs": ["H0", "Q0", "n", "nut", "B", "H"],
    },
...
class ResultsLoader:
    ...
    def _setup_datasets(self):
        ...
        if os.path.exists(self.config.data.parameters_path):
            ...
            self.full_dataset = train_val_dataset
        else:
            ...
            if test_size == 0 or train_val_size == 0:
                raise ValueError(
                    "Dataset is too small for the specified train/test split."
                )
            ...
            self.full_dataset.compute_statistics(train_val_dataset.indices)
