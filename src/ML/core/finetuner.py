...
from ML.core.results import GEOM_NAMES, GEOMETRY_FILES, TRAINED_MODELS_INFO
...
class ModelFineTuner:
    ...
    def _get_model_class(self):
        ...
        model_name = self.base_config.model.class_name
        try:
    ...
    def finetune(self) -> float | None:
        ...
        try:
            ...
            if os.path.exists(ft_config.data.parameters_path):
                train_val_dataset = HDF5Dataset.from_config(
                    ft_config,
                    file_path=ft_config.data.file_path,
                    filter_set_type="train_val",
                )
                test_dataset = HDF5Dataset.from_config(
                    ft_config,
                    file_path=ft_config.data.file_path,
                    filter_set_type="test",
                )
            else:
                ...
                test_size = int(
                    ft_config.training.test_frac * len(full_dataset_for_stats)
                )
            ...
            # Instantiate model using the source model architecture parameters
            # merged with fine-tuning training hyperparameters (learning rate, batch size)
            ...
            if isinstance(train_val_dataset, torch.utils.data.Subset):
                base_train_val.compute_statistics(train_val_dataset.indices)
            else:
                base_train_val.compute_statistics()

            if base_test is not base_train_val and hasattr(
                base_test, "copy_statistics_from"
            ):
                base_test.copy_statistics_from(base_train_val)
            ...
            criterion = PhysicsInformedLoss(
                ...
                config=ft_config,
                dataset=base_train_val,  # Stats from TARGET dataset
