...
from ML.core.finetuner import ModelFineTuner
...
def main():
    ...
    data_dir = project_root / base_config_template.paths.data_dir
    jobs_attempted = 0
    jobs_succeeded = 0
    ...
    for model_key, model_info in TRAINED_MODELS_INFO.items():
        ...
        for target_geom_char, target_data_file_suffix in GEOMETRY_FILES.items():
            if (
                target_geom_char.startswith("test_")
                or target_geom_char == source_geom_char
            ):
                continue  # Skip test sets and fine-tuning on the original geometry

            jobs_attempted += 1
            target_data_file = str(data_dir / target_data_file_suffix)
