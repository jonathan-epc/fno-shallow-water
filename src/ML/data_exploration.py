...
def extract_data(dataset, config):
    ...
    for data in dataset:
        ...
        input_keys = [
            var for var in config.data.inputs if var not in dataset.SCALARS
        ] + [var for var in config.data.inputs if var in dataset.SCALARS]
        output_keys = [
            var for var in config.data.outputs if var not in dataset.SCALARS
        ] + [var for var in config.data.outputs if var in dataset.SCALARS]

        for key, value in zip(input_keys, field_inputs + scalar_inputs, strict=False):
            input_data[key].append(value)
        for key, value in zip(
            output_keys, field_outputs + scalar_outputs, strict=False
        ):
