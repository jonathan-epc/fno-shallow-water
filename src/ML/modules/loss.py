...
class PhysicsInformedLoss(nn.Module):
    ...
    def reset(self) -> None:
        """Reset ReLoBRaLo loss weights, loss history, and step counts."""
        self.lambdas.fill_(1.0)
        self.last_losses.fill_(1.0)
        self.init_losses.fill_(1.0)
        self.call_count = 0
    ...
    def forward(
        ...
        device = (
            ...
            if field_pred is not None
                ...
                if scalar_pred is not None
                    ...
                    inputs[0][0].device
                    if inputs[0]
                    else (inputs[1][0].device if inputs[1] else torch.device("cpu"))
                )
            )
        )
        ...
        if field_pred is not None and field_target:
            ...
            total_data_loss = total_data_loss + self.data_loss(
                field_pred, field_target_tensor
            )
        ...
        if scalar_pred is not None and scalar_target:
            ...
            total_data_loss = total_data_loss + self.data_loss(
                scalar_pred, scalar_target_tensor
            )
