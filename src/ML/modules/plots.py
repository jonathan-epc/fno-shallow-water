...
def plot_scatter_predictions(
    ...
    if separate_plots:
        ...
        for i, name in enumerate(all_names):
            ...
            pred = all_preds[i].detach().cpu().numpy().flatten()
            targ = all_targs[i].detach().cpu().numpy().flatten()
...
def plot_error_analysis(
    ...
    for i, name in enumerate(field_names):
        pred_np = field_preds[:, i].detach().cpu().numpy()
        targ_np = field_targs[:, i].detach().cpu().numpy()
