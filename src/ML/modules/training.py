        ...
        if checkpoint_to_load is not None:
            try:
                ...
                if (
                    ...
                    # Strip checkpoint metadata key if present
                    final_state_dict = {
...
def cross_validate(
    ...
    for fold, (train_idx, val_idx) in fold_pbar:
        ...
        # --- Reset Early Stopping & Loss Criterion State for the Fold ---
        ...
        if hasattr(criterion, "reset"):
            criterion.reset()
        logger.debug(
            f"Fold {fold_num}: Early stopping and criterion state reset for the fold."
        )
