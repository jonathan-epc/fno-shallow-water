    ...
    for bottom_type in bottom_types:
        if config.separate_critical_states:
            for critical_state in ["subcritical", "supercritical"]:
                ...
                filtered_ids = set(filtered_parameters.index.astype(str))
                filtered_result_files = [
                    f for f in result_files if os.path.splitext(f)[0] in filtered_ids
                ]

                with h5py.File(
                    files[bottom_type][f"main_{critical_state}"], "w"
                ) as hdf5_file:
                    state_table = process_and_save(
                        hdf5_file,
                        bottom_type.upper(),
                        config.base_dir,
...
def process_simulation_results(config: ProcessingConfig, yaml_config: dict) -> None:
    ...
    try:
        ...
        if config.set_type:
            ...
            # Fallback for unpartitioned datasets
            set_types = [None]
            logger.info(
